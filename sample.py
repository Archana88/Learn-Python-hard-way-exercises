...
def map_x(point,f):
    ...
    # can employ any method to label the data set. here for all points where: x>y its mapped to +1, and -1 otherwise
    y = f(x1)
    compare_to = y1
...
such that t_set[index] is misclassified <=> yn != sign(w*point)'''
    ...
    for i in range(len(t_set)):
        ...
        yn = t_set[i][1]
        if s != yn:
            res = res + (i,) # adds the index 'i' to the res tuple, res consists of indexes of all mis classified data sets
    return res
...
'''
    ...
    f = lambda x: l[0]*x + l[1] # is a way to define a function in one line. this is a line equation
...
def run_PLA(N_samples,N_points):
    ...
    plt.plot(range(1,N_samples),diff, 'b')
    plt.axis(1,max(diff),1, N_samples)
